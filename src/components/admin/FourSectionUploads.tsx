...
export const uploadGroups: {id:string;role:AssetRole;label:string;accept:string;sourceKind?:"schematic"|"layout";captions?:"layout"|"photos";single?:boolean}[] = [
  {id:"schematic-source",role:"source",sourceKind:"schematic",label:"Schematic source files (.SchDoc)",accept:".SchDoc"},
  {id:"schematic-pdf",role:"schematic-pdf",label:"Schematic PDF",accept:".pdf",single:true},
  {id:"schematic-images",role:"schematic",label:"Schematic images",accept:images},
  {id:"layout-source",role:"source",sourceKind:"layout",label:"PCB layout source files (.PcbDoc)",accept:".PcbDoc"},
  {id:"layout-pdf",role:"layout-pdf",label:"PCB layout PDFs",accept:".pdf"},
  {id:"layout-images",role:"layout",label:"PCB layout images",accept:images,captions:"layout"},
  {id:"render",role:"render",label:"3D render images",accept:images},
  {id:"photo",role:"photo",label:"Physical photo images",accept:images,captions:"photos"},
];
...
const sections = [
  {title:"Schematic",description:"Upload .SchDoc source files. Add an optional PDF or images for the public preview.",groups:["schematic-source","schematic-pdf","schematic-images"]},
  {title:"PCB Layout",description:"Upload .PcbDoc source files. Add optional layout images or PDFs for the public preview.",groups:["layout-source","layout-pdf","layout-images"]},
  {title:"3D Render",description:"Add rendered images of the board. You can select several images at once.",groups:["render"]},
  {title:"Physical Photos",description:"Add photos of the assembled board, with optional captions.",groups:["photo"]},
];
...
export function FourSectionUploads({pending,onPending,assets,board,selectedUrls,onRemove,onCaption,archived}: {
  pending:PendingUploads;onPending:(value:PendingUploads)=>void;assets:Asset[];board:Board;selectedUrls:Set<string|undefined>;
  onRemove:(url:string)=>void;onCaption:(field:"layout"|"photos",src:string,caption:string)=>void;archived:boolean;
}) {
  return <div className="admin-upload-sections">{sections.map((section,index)=><section className="admin-upload-section" key={section.title} aria-labelledby={`upload-section-${index}`}>
    ...
    {section.groups.map(id=>{
      ...
      const saved=assets.filter(a=>a.role===group.role && (group.sourceKind ? !a.superseded && a.name.toLowerCase().endsWith(group.sourceKind==="schematic"?".schdoc":".pcbdoc") : selectedUrls.has(a.url)));
      return <div className="admin-upload-field" key={id}>
        {!archived && <><label>{group.label}<input type="file" accept={group.accept} multiple={!group.single} onChange={e=>{
          ...
          if(added.length) onPending({...pending,[id]:{...selection,files:group.single?added:[...selection.files,...added]}});
          e.target.value="";
        }} /></label>
        ...
        {(saved.length>0 || selection.files.length>0) && <div className="admin-upload-files">
          {saved.map(asset=><div className="admin-upload-file" key={asset.id}>
            ...
            <div><a href={assetUrl(asset.url)} target="_blank" rel="noreferrer">{asset.name}</a><p className="muted">{group.sourceKind?"Private source file":"Saved to draft"}</p>
            ...
            {!archived && !group.sourceKind && <button type="button" className="text-button" onClick={()=>onRemove(asset.url)}>Remove from draft</button>}</div>
          </div>)}
