...
export function Catalog({ boards }: { boards: BoardSummary[] }) {
  ...
  const category = boardCategories.find(c => c.id === filters.category);
  ...
  const result = invalid ? [] : filterBoards(boards, filters);
  ...
  return <>
    <div className="page-heading"><p className="eyebrow">The archive</p><h1>{category?.label || "All PCB works"}</h1><p className="lead">{category?.description || "Explore designs, technical drawings, models, and board documentation."}</p></div>
    <div className="filter-panel">
      ...
      <div className="filter-row">
        <label>Category<select value={filters.category} onChange={e => update("category", e.target.value)}><option value="">All categories</option>{filters.category && !category && <option value={filters.category}>Unknown category</option>}{boardCategories.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</select></label>
    ...
    {result.length ? <BoardGrid boards={result} /> : <div className="empty-state"><h2>No PCB works match the current filters.</h2><p>Try a different search or clear the filters.</p></div>}
  </>;
}
