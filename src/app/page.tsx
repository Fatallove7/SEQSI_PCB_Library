...
export default function Home() {
  ...
  return <>
    <section className="home-intro"><p className="eyebrow">Engineering archive</p><h1>{site.title}</h1><p className="lead">{site.description}<br />From schematic to assembled board, in one place.</p>
      ...
      <p className="archive-stats">{boards.length} PCB works <span>·</span> {categories.length} categories <span>·</span> An evolving design archive</p>
    </section>
    ...
    <section className="home-section"><div className="section-heading"><h2>Browse by category</h2><Link href="/categories">All categories ↗</Link></div><div className="category-grid">{categories.map(c => <Link className="category-card" key={c.id} href={`/boards?category=${encodeURIComponent(c.id)}`}><div><h3>{c.name}</h3><p>{boards.filter(b => b.category === c.id).length} boards</p></div><span aria-hidden="true">↗</span></Link>)}</div></section>
    ...
    <section className="home-section"><div className="section-heading"><div><h2>Latest boards</h2><p className="muted">Recently added to the library.</p></div><Link href="/boards">View all boards ↗</Link></div>{latest.length ? <BoardGrid categories={categories} boards={getBoardSummaries(latest)} /> : <p className="empty-state">No PCB works have been added yet.</p>}</section>
  </>;
}
