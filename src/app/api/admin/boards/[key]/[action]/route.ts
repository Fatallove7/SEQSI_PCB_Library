...
import { boundedBody, MAX_UPLOAD_BYTES, prepareLibraryUploads } from "@/lib/admin/uploads";
...
export async function POST(request: Request, {params}: {params:Promise<{key:string;action:string}>}) {
  try {
    ...
    const permissions: Record<string,Permission> = {upload:"upload",publish:"publish",archive:"archive",restore:"restore"};
    ...
    if (action === "upload") {
      ...
      const role = String(form.get("role")) as AssetRole;
      ...
      const prepared = await prepareLibraryUploads(incoming,role);
      const record = repo.addAssets(key,version,prepared,form.get("replace") === "true",user.username);
