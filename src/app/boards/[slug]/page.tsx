...
export default async function BoardDetail({ params }: Props) {
  ...
  const sources = managed ? repository().assets(managed.key).filter(a=>a.role==="source"&&!a.superseded) : [];
  const sourceLinks = (extension:string) => sources.filter(a=>a.name.toLowerCase().endsWith(extension)).map(a=><a className="button" key={a.id} href={assetUrl(a.url)}>Source: {a.name}</a>);
  const category = boardCategories.find(c => c.id === board.category);
  const downloads = [...(board.downloads || [])];
  for (const pdf of board.layoutPdfs || []) if (!downloads.some(d=>d.file===pdf.file)) downloads.push({file:pdf.file,label:pdf.label || "PCB layout PDF"});
  ...
  const sections = [["schematic", "Schematic"], ["layout", "PCB layout"], ["model", "3D"], ["photos", "Physical board"], ["specifications", "Specifications"], ["downloads", "Downloads"]];
  return <>
    ...
    <div className="detail-layout"><aside className="section-nav"><p className="eyebrow">On this page</p><nav aria-label="Board sections">{sections.map(([id, label]) => <a key={id} href={`#${id}`}>{label}</a>)}</nav><Link className="back-link" href="/boards">← All boards</Link></aside><div className="detail-content">
      <section className="detail-section" id="schematic"><div className="section-heading"><h2>Schematic</h2>{board.schematic?.pdf && <a className="button" href={assetUrl(board.schematic.pdf)} target="_blank" rel="noreferrer">Open PDF ↗</a>}{sourceLinks(".schdoc")}</div>{board.schematic?.images?.length ? <ImageGallery label="Schematic" images={board.schematic.images.map((src, i) => ({ src, caption: `Schematic — page ${i + 1}` }))} /> : !board.schematic?.pdf && <p className="missing">{board.sourceAvailability?.schematic ? "Preview not generated yet" : "Preview not available"}</p>}</section>
      <section className="detail-section" id="layout"><div className="section-heading"><h2>PCB layout</h2>{(board.layoutPdfs || []).map(pdf=><a key={pdf.file} className="button" href={assetUrl(pdf.file)} target="_blank" rel="noreferrer">{pdf.label || "Open layout PDF"} ↗</a>)}{sourceLinks(".pcbdoc")}</div>{board.layout?.length ? <ImageGallery label="PCB layout" images={board.layout} /> : !board.layoutPdfs?.length && <p className="missing">{board.sourceAvailability?.layout ? "Preview not generated yet" : "Preview not available"}</p>}</section>
      <section className="detail-section" id="model"><h2>3D</h2><ModelViewer data={board.model3d} title={board.title} /></section>
